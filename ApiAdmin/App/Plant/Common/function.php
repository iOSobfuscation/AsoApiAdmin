...
function order_sum($type, $where_sum){
	...
	$where		   .= " and user_id = ".session('userid').$where_sum;//.session('userid');
	$db				= D('order');
...
function order_list($type){
	...
	$where		   .= " and user_id = ".session('userid');
...
function coupon_info($id){
	...
	$userid						= session('userid');
	$db							= D('coupon_record');
	$res						= '<a href="/Plant/Flow/coupon_list/"><div class="y-title icon-coupon" style="margin-top:8px; text-indent:26px; font-size:16px; height:30px; line-height:30px; ">';
	if(empty($id)){
		//判断用户是否有可用的优惠券
...
function coupon_list(){
	...
	$where		= "status = 0 and user_id = ".session('userid');
		...
		if($status == 0){
			...
			if($pay_type == 1){
				...
				if($order_pay == 1){
					...
					//未支付,调用支付链接
					$list[$key]['p_url']				= "/Plant/flow/order_pay/order_sn/$order_sn";
					$list[$key]['url_name']				= "立即支付";
				}elseif($order_pay == 0){
					$list[$key]['p_url']				= "/Plant/Prize/receipt/actr_id/".$val['id'];
			...
			//未领取
			$list[$key]['p_url']						= "/Plant/Prize/receive/actr_id/".$val['id'];
...
function pc_record($userid, $where){
	...
	$where		= " userid = $userid".$where;// and start_time <= '$now' <= end_time ";
