...
function is_complete($info){
	...
	$db   	= M('users', 'mp_');
	$where	= "status = 0 and id = ".session('userid');
	$info	= "signature, tel";
 ...
 * return string
 */
function qrcode($content,$tpgs,$qrcode_bas_path=false,$errorCorrectionLevel='L',$matrixPointSize='4',$matrixMarginSize='1'){
	//生成二维码
...
function act_fixed_goods($act_id){
	...
	$goods_res			= $goods_db->where($gwhere)->order("position asc")->select();
...
function act_random_goods($act_id, $position){
	...
	//获取活动所有随机商品ID
	$random_goods			= $act_res['random_goods'];
	$random_goods_arr		= explode(',', $random_goods);//转数组
	...
	if($act_id == ''){
		...
		$rec_where			= "activite = $act_id and userid = $userid and sign_time >= $start_time and sign_time <= $end_time";
	}
	//先取所有获得的商品ID
	$rec_res				= $act_record_db->where($rec_where)->getField('goods_id', true);
	
	
	if(empty($rec_res)){
		//无中奖记录
		$goods_ids			= $random_goods;
	}else{
		//去重
		$rec_res			= array_flip(array_flip($rec_res));
		$rgc_id_str			= implode(',', $rec_res);//组成id串
		
		//获取所有的已中随机商品ID
		$recgl_where		= "cat_id = 40 and id in($rgc_id_str)";
		$rgc_id_str			= $goods_db->where($recgl_where)->getField('id', true);
		//echo $goods_db->getlastsql();
		if(empty($rgc_id_str)){
		//无中奖记录
			$goods_ids		= $random_goods;
		}else{
			$rgc_id_str		= implode(',', $rgc_id_str);//组成id串
		}
		//echo $rgc_id_str;
		//exit;
		
		if($random_goods === $rgc_id_str){
			//相等则重新计算
			$goods_ids		= $rgc_id_str;
		}else{
			//求差集合
			$ragids			= array_diff($random_goods_arr, $rec_res);
			$goods_ids		= implode(',', $ragids);
			//求长度
			$goods_ids_num	= substr_count($goods_ids,",");
			if($goods_ids_num == 0){
				$rand_gids	= array_rand($rec_res, 2);
				$gids		= array_merge_recursive($ragids, array('0'=>$random_goods_arr[$rand_gids[0]]), array('0'=>$random_goods_arr[$rand_gids[1]]));
				$goods_ids	= implode(',', $gids);
			}elseif($goods_ids_num == 1){
				$rand_gids	= array_rand($rec_res, 1);
				$gids		= array_merge_recursive($ragids, array('0'=>$random_goods_arr[$rand_gids]));
				$goods_ids	= implode(',', $gids);
			}
	...
	$gwhere					= "id in (".$goods_ids.") and stock > 10";
	$goods_res				= $goods_db->field('id,thumb,name,position,cat_id')->where($gwhere)->limit(3)->group('position')->select();
	
	//$position				= 7;
	foreach($goods_res as $key => $val){
		$goods_list[$key]['id']			= $val['id'];
		$goods_list[$key]['position']	= $val['position'];
		$goods_list[$key]['thumb']		= $val['thumb'];
		$goods_list[$key]['cat_id']		= $val['cat_id'];
		//$position						= $position+4;
	}
	//echo $goods_db->getlastsql();
	return $goods_list;
...
function handle_act($act_id, $goods_id, $prize_type){
	...
	$data['userid']					= $userid;
	$data['sign_time']				= time();
	$data['expiration_time']		= $data['sign_time']+604800;//过期时间
	$data['status']					= 1;
	$data['deliver']				= 1;
	...
	//$log_data['msg']			= $act_record_db->getlastsql()."||||".$$prize_type;
	//$log_db->add($log_data);
	...
	if($actr_id == ''){
		...
		//扣除会员余额
		$member_db->where(array('id'=>$userid))->setDec('balance'); // 用户的余额减1
		$log_db						= D('error');
			
		$log_data['msg']			= $member_db->getlastsql()."||||".$actr_id."||||".$userid;
		$log_db->add($log_data);
		
		//增加活动已玩次数
		$act_db->where(array('id'=>$act_id))->setInc('sum_play',1); 
		
		//商品中奖数
		$goods_db->where(array('id'=>$goods_id))->setInc('end_num',1); 
		...
		if($acmn_res){
			$activite_number_db->where($acmn_where)->setInc('number',1); // 当日娱乐次数+1
		}else{
			...
			$activite_number_db->add($cn_data); // 插入记录+1
		}
	}
	...
	$po_data						= '{
										   ...
										   "data":{
												"first": {"value":"亲爱的 '.$nick.',你好！恭喜您获得'.$act_name.$goods_name.'","color":"#173177"},
...
function getcode($eid=1, $id){
	...
	$matrixMarginSize 		= 1;//边距大小
	$data_code['code']		= "/".qrcode($content,$tpgs,$qrcode_bas_path,$errorCorrectionLevel,$matrixPointSize,$matrixMarginSize);
...
function probability_winning($cId1,$cId2,$cId3){
	...
	$userid				= session('userid');	
	$activite_res		= $activite_db->field('sum_play,fixed_goods,random_goods,will_goods')->where("id=20")->find();
	
	$activite_number_db = D("activite_number");
	$userid = session('userid');
	$time = time();
	$played = $activite_number_db->field('number')->where("userid={$userid} and activiteid=20 and start_time<='{$time}' and end_time>='{$time}'")->find();
...
function settlement_money($userid, $amount = 1){
	...
	if($fm_id != 0){
		...
		if($fm_agencylevel != 1){
			...
			//变更分销商的未提现金额
			$member_mentioned_res			= $member_db->where(array('id'=>$fm_id))->setInc('not_mentioned', $fm_not_mentioned);
			//记录提成记录
	...
	if($gm_id != 0){
		...
		if($agencylevel == 2){
			...
			//变更分销商的未提现金额
			$mentioned_res				= $member_db->where(array('id'=>$gm_id))->setInc('not_mentioned', $gm_not_mentioned);
	...
	if($fgm_id != 0){
		...
		if($agencylevel == 2){
			...
			//变更分销商的未提现金额
			$mentioned_res				= $member_db->where(array('id'=>$fgm_id))->setInc('not_mentioned', $fgm_not_mentioned);
	...
	if($comra_res && $comrfm_res && $comr_res && $comrfgm_res){
